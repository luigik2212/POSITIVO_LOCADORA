...
class NotificationCenter extends BaseModel
{
    ...
    public function allActive(int $userId, int $windowDays = 15, ?int $limit = null, int $offset = 0): array
    {
        $notifications = $this->buildActiveNotifications($userId, $windowDays);

        if ($limit === null) {
            return $notifications;
        }

        return array_slice($notifications, max(0, $offset), max(1, $limit));
    }

    public function paginated(int $userId, int $page = 1, int $perPage = 20, int $windowDays = 15): array
    {
        $safePerPage = max(1, min(100, $perPage));
        $safePage = max(1, $page);
        $all = $this->buildActiveNotifications($userId, $windowDays);
        $total = count($all);
        ...
        return [
            'items' => array_slice($all, $offset, $safePerPage),
            'total' => $total,
    ...
    private function buildActiveNotifications(int $userId, int $windowDays = 15): array
    {

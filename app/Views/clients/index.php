...
<button class="btn btn-sm btn-warning" onclick='openClientModal(<?= json_encode($c, JSON_HEX_APOS|JSON_HEX_QUOT) ?>)'>Editar</button>
<a class="btn btn-sm btn-info" href="/clients?client_id=<?= $c['id'] ?>">Histórico</a>
...
<div class="modal fade" id="clientModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><form method="POST" action="/clients/store" id="clientForm"><div class="modal-header"><h5>Cliente</h5></div><div class="modal-body row g-2">
<input type="hidden" name="_token" value="<?= csrfToken() ?>"><input type="hidden" name="id" id="client_id">
<?php $fields=['nome_completo','cpf','rg','cnh','validade_cnh','telefone','email']; foreach($fields as $f): ?><div class="col-md-6"><label class="form-label"><?= ucfirst(str_replace('_',' ',$f)) ?></label><input class="form-control" name="<?= $f ?>" id="c_<?= $f ?>" <?= in_array($f,['nome_completo','cpf'])?'required':'' ?>></div><?php endforeach; ?>

...
<?php foreach($clients as $c): $clientJson = $c; $clientJson['documents'] = $documentsByClient[(int)$c['id']] ?? []; ?><tr><td><?= esc($c['nome_completo']) ?></td><td><?= esc($c['cpf']) ?></td><td><?= esc($c['telefone']) ?></td><td><?= esc($c['email']) ?></td><td>
...
<div class="modal fade" id="clientModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><form method="POST" action="<?= url('/clients/store') ?>" id="clientForm" enctype="multipart/form-data"><div class="modal-header"><h5>Cliente</h5></div><div class="modal-body row g-2">
...
<div class="col-12"><label class="form-label">Anexar documentos</label><input class="form-control" type="file" name="documentos[]" multiple></div>
<div class="col-12 d-none" id="clientDocumentsWrap">
  <label class="form-label mb-1">Documentos anexados</label>
  <div id="clientDocumentsList" class="list-group list-group-flush border rounded"></div>
</div>
